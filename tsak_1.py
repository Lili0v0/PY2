...
class Cup:    # TODO Написать 3 класса с документацией и аннотацией типов
...
class PC:
    ...
    def __init__(self, diagonal: Union[int, float], mark: str, volume: int):
        """
        Создание и подготовка к работе объекта класса PC
        :param diagonal: диагональ экрана PC;
        :param mark: бренд PC;
        :param volume: громкость PC;
        Примеры:
        >>> Computer = PC(40,"Phillips",12) # инициализация объекта
        """
        if diagonal < 0:
            raise ValueError("Диагональ экрана PC должна быть положительным числом")
        if not isinstance(diagonal, (int, float)):
            raise TypeError("Диагональ экрана PC должна быть int или float")
        self.diagonal = diagonal

        if not isinstance(mark, str):
            raise TypeError("Бренд PC должен быть типа str")
        self.mark = mark

        if volume < 0:
            raise ValueError("Громкость PC должна быть положительным числом")
        if not isinstance(volume, int):
            raise TypeError("Громкость PC должен быть типа int")
        self.volume = volume

    def increase_the_volume(self, volume: int) -> None:
        """Метод, который увеличивает громкость PC
        :raise ValueError: Если значение громкости больше возможного для данного PC, то вызовем ошибку
        Примеры:
         >>> Computer = PC(40,"Philips",12)
         >>> Computer.increase_the_volume(18)
         """
        if volume < 0:
            raise ValueError("Громкость PC должна быть положительным числом")
        if not isinstance(volume, int):
            raise TypeError("Новое значение громкости PC должно быть типа int")
        self.volume = volume

    def turn_on_the_PC(self) -> None:
        """Метод, который включает PC
        Примеры:
        >>> Computer = PC(40,"Philips",15)
        >>> Computer.turn_on_the_PC()
        """
        ...
